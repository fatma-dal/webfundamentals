...
app.get("/tvshows",(req,res)=>{res.json(tvShows)} )
app.get("/tvshows/:year",(req,res)=>{
    const getTvByYear=tvShows.find(show=> show.yearCreated==req.params.year)
    console.log(getTvByYear)
    res.json(getTvByYear)

})
app.delete("/tvshows/:title",(req,res)=>{
    const showTitle=req.params.title
    const deletedshow = tvShows.find(show => show.tvShow == showTitle)
    const index = tvShows.indexOf(deletedshow)
...
app.patch('/tvshows/:title', (req, res) => {
    ...
    let newStarring = req.body.starring;
    let show = tvShows.find(show => show.tvShow === title);
    let index = tvShows.indexOf(newStarring)
    Object.assign(newStarring,req.body)
    tvShows[index]=newStarring
